...
### A Data Science Approach to Maintaining Water Sources.pptx
...
Our Numerical Data has lots of 0s. We will assume these are meant to be empty fields (NaN)
...
Different regions in Tanzania have a different ratio of fully-working pumps to pumps that need maintenance.
...
We then define our pipeline so that ALL DATA TRANSFORMATION (other than for the target variable) is done in pipeline.
...
We try to optimize our hyperparameters for Random Forest through a Randomized Search procedure.

Performance is still worse than baseline!

## Gradient Boosting Classifier

Next we try a Gradient Boosting Classifier and that improves our score by over two percentage points.

# Summary

We have created a model that can predict whether or not a pump needs maintenance about 80% of the time.

To improve the model, we could try paring down the model features through lasso regression.

For our final product though, we wouldn't use the classifier, but rather a list of each water source with its probability of being in class 1. That way, we could create a prioritized list of pumps that likely need to be maintained.
